...
export default function TripItem({ item }) {
  ...
  const handlePayment = async () => {
    ...
    if (!loggedInUser || !loggedInUser.id) {
      Alert.alert('Chưa đăng nhập', 'Vui lòng đăng nhập để tiếp tục thanh toán.');
      return;
    }
    ...
    try {
      ...
      const checkoutParams = {
        ...
        bookingId: item._id,
        fullName: item.fullName || `${loggedInUser.lastName} ${loggedInUser.firstName}`,
      ...
      Alert.alert('Đã xảy ra lỗi', 'Không thể tiến hành thanh toán. Vui lòng thử lại.');
    } finally {
      setIsPaying(false);
    }
  };
      ...
      >
        ...
        >
          <View style={styles.statusBadge}>
            ...
            <Text style={styles.statusText}>{isPaying ? 'ĐANG XỬ LÝ...' : currentStatus.text}</Text>
          </View>
        ...
        <View style={styles.cardFooter}>
          {item.status === 'pending' ? (
            ...
            <TouchableOpacity
              ...
              disabled={isPaying}
            >
              <Text style={styles.actionButtonText}>{isPaying ? 'Đang xử lý...' : 'Thanh toán ngay'}</Text>
