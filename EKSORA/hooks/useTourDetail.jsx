...
export const useTourDetail = (productId) => {
  ...
  const onBookNow = () => {
    ...
    const selectedOptionsDetails = Object.entries(currentSelectedPackages).map(
      ([packageId, optionId]) => {
        const pkg = productData.availableServicePackages.find(
          (p) => p.id === packageId
        );
        ...
        return {
          packageId,
          optionId,
          title: pkg?.name || "Dịch vụ không xác định",
          optionName: option?.title || "Tùy chọn không xác định",
