...
const QuantitySelector = ({ label, quantity, onUpdate, price }) => (
  <View style={styles.quantitySelector}>
    <View>
      ...
      {price > 0 && <Text style={styles.priceText}>{formatCurrency(price)}</Text>}
    </View>
...
const CartItem = ({ item, isSelected, onToggleSelect, onDelete, onPressItem }) => {
  const { updateCartItem } = useCart();

  if (!item) return null;

  const originalItemPrice = (item.originalAdultPrice * item.adults) + (item.originalChildPrice * item.children);
  const hasDiscount = item.discount > 0;

  const handleUpdateQuantity = (type, amount) => {
    let newAdults = type === 'adults' ? Math.max(1, item.adults + amount) : item.adults;
    let newChildren = type === 'children' ? Math.max(0, item.children + amount) : item.children;
    const newTotalPrice = (item.adultPrice * newAdults) + (item.childPrice * newChildren);
  ...
  return (
    ...
    <TouchableOpacity style={styles.cardWrapper} onPress={onPressItem} activeOpacity={0.8}>
      ...
      <View style={styles.cardContent}>
        ...
        <TouchableOpacity onPress={(e) => { e.stopPropagation(); onToggleSelect(item.id); }} style={styles.checkboxContainer}>
        ...
        <View style={styles.infoContainer}>
          ...
          {item.selectedOptions && item.selectedOptions.length > 0 && (
            <View style={styles.optionsContainer}>
              {item.selectedOptions.map((opt, index) => (
                <Text key={index} style={styles.optionText} numberOfLines={1}>+ {opt.optionName}</Text>
              ))}
            </View>
          )}
            ...
            <View style={styles.voucherInfoContainer}>
              <View style={styles.voucherRow}>
                ...
                <Text style={styles.voucherText} numberOfLines={1}>
                  Mã <Text style={styles.voucherCode}>{item.voucherCode}</Text> giảm <Text style={styles.voucherAmount}>-{formatCurrency(item.discount)}</Text>
                </Text>
              </View>
              <View style={styles.voucherRow}>
          ...
          <View style={styles.footerRow}>
            <View style={styles.priceSection}>
              <Text style={styles.finalPrice}>{formatCurrency(item.price)}</Text>
              {hasDiscount && <Text style={styles.originalPrice}>{formatCurrency(originalItemPrice)}</Text>}
            </View>
            <View style={styles.actionsContainer}>
              <TouchableOpacity onPress={(e) => { e.stopPropagation(); onDelete(item.id); }} style={styles.actionButton}>
